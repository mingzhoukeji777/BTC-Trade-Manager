...
public sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BTC交易管理系统 v0.6.3"; Font=new Font("Microsoft YaHei",9); StartPosition=FormStartPosition.CenterScreen; MinimumSize=new Size(1120,680); Size=new Size(1360,820); LoadUi(); BuildUi(); RefreshAll();
    ...
    static DataGridView Grid(){
        var g=new DataGridView{Dock=DockStyle.Fill,Font=new Font("Microsoft YaHei",9),DefaultCellStyle=new DataGridViewCellStyle{Font=new Font("Microsoft YaHei",9)},ColumnHeadersDefaultCellStyle=new DataGridViewCellStyle{Font=new Font("Microsoft YaHei",9,FontStyle.Bold)},ReadOnly=true,AllowUserToAddRows=false,AllowUserToDeleteRows=false,RowHeadersVisible=false,AutoSizeColumnsMode=DataGridViewAutoSizeColumnsMode.Fill,SelectionMode=DataGridViewSelectionMode.FullRowSelect,MultiSelect=false,AutoGenerateColumns=true};
    ...
    void BuildUi()
    {
        var root=new TableLayoutPanel{Dock=DockStyle.Fill,RowCount=2,ColumnCount=1}; root.RowStyles.Add(new RowStyle(SizeType.Absolute,50)); root.RowStyles.Add(new RowStyle(SizeType.Percent,100)); Controls.Add(root); Controls.Add(_status); _tabs.DrawMode=TabDrawMode.OwnerDrawFixed; _tabs.Padding=new Point(18,4); _tabs.DrawItem+=DrawTab;
    ...
    ContextMenuStrip MainMenu(){ var m=new ContextMenuStrip(); m.Items.Add("修改",null,(_,_)=>EditMain(RowId(_mainGrid))); m.Items.Add("删除",null,(_,_)=>DeleteMain()); m.Items.Add("移动止盈",null,(_,_)=>SetMainProtection("移动止盈")); m.Items.Add("成本止损",null,(_,_)=>SetMainProtection("成本止损")); m.Items.Add("平仓",null,(_,_)=>CloseMain()); return m; }
    ContextMenuStrip NodeMenu(){ var m=new ContextMenuStrip(); m.Items.Add("修改",null,(_,_)=>EditNode(CurrentNodeId())); m.Items.Add("删除",null,(_,_)=>DeleteNode()); m.Items.Add("移动止盈",null,(_,_)=>SetProtection("移动止盈")); m.Items.Add("成本止损",null,(_,_)=>SetProtection("成本止损")); m.Items.Add("平仓",null,(_,_)=>CloseNode()); m.Items.Add("减仓",null,(_,_)=>ReduceCurrentNode()); return m; }
    ...
    void RefreshLiveOnly(){ LoadRisk(); BuildSummary(); }
    void LoadMainGrid(){ _mainGrid.DataSource=_store.Query("SELECT id,code AS 编号,direction AS 方向,CASE cycle_name WHEN '牛市周期' THEN '牛' WHEN '熊市周期' THEN '熊' ELSE cycle_name END AS 周期,status AS 状态,base_qty AS 持仓量,avg_price AS 开仓均价,breakeven_price AS 盈亏平衡价,liq_price AS 强平价格,floating_pnl AS 浮动收益,maintenance_margin_rate AS 维持保证金率,margin AS 保证金,hard_stop AS 止损价格,cost_stop AS 成本止损,trailing_take_profit AS 移动止盈,funding_rate AS 资金费率,fee AS 手续费,cumulative_funding AS 累计资金费,cumulative_fee AS 累计手续费,close_price AS 平仓价格,created_at AS 开仓时间,close_time AS 平仓时间,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注 FROM main_positions WHERE account_key=$a ORDER BY id",("$a",AccountKey)); HideId(_mainGrid); }
    void LoadNodeGrid(){
        var cols="SELECT id,code AS 编号,direction AS 方向,status AS 状态,qty AS 持仓量,entry_price AS 开仓均价,breakeven_price AS 盈亏平衡价,liq_price AS 强平价格,floating_pnl AS 浮动收益,maintenance_margin_rate AS 维持保证金率,0 AS 保证金,COALESCE(stop_loss,close_price) AS 止损价格,cost_stop AS 成本止损,trailing_take_profit AS 移动止盈,funding_rate AS 资金费率,fee AS 手续费,close_exit_price AS 平仓价格,created_at AS 开仓时间,close_time AS 平仓时间,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注 FROM position_nodes WHERE account_key=$a";
        ...
        var reduce="SELECT id,code AS 编号,direction AS 方向,status AS 状态,qty AS 减仓量,CASE WHEN floating_pnl>=0 THEN '盈' ELSE '亏' END AS 盈亏,floating_pnl AS 盈亏金额,entry_price AS 开仓均价,close_exit_price AS 减仓价格,fee AS 手续费,created_at AS 开仓时间,close_time AS 平仓时间,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注 FROM position_nodes WHERE account_key=$a AND node_type='减仓' ORDER BY id DESC";
        _reduceGrid.DataSource=_store.Query(reduce,("$a",AccountKey)); HideId(_reduceGrid);
        var closed="SELECT id,code AS 编号,direction AS 方向,status AS 状态,qty AS 持仓量,CASE WHEN floating_pnl>=0 THEN '盈' ELSE '亏' END AS 盈亏,floating_pnl AS 盈亏金额,entry_price AS 开仓均价,close_exit_price AS 平仓价格,fee AS 手续费,created_at AS 开仓时间,close_time AS 平仓时间,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注 FROM position_nodes WHERE account_key=$a AND status='已平仓' ORDER BY id DESC";
    ...
    static string StatusBall(string? s)=>s=="已开仓"||s=="持有中"?"🔴":s=="成本止损"?"🟡":s=="移动止盈"?"🟢":"⚪";
    ...
    void CloseMain(long? forcedId=null){ var id=forcedId??RowId(_mainGrid); if(id==null)return; using var f=new SimpleComboForm("平仓",new FieldSpec("平仓价格","text"),new FieldSpec("平仓订单号","text")); if(f.ShowDialog(this)==DialogResult.OK){ _store.Exec("UPDATE main_positions SET close_price=$p,exchange_close_order_id=$o,close_time=datetime('now','localtime'),status='已平仓' WHERE id=$id",("$p",DbNum(f.Values[0])),("$o",f.Values[1]),("$id",id)); RefreshAll(); }}
    void CloseNode(long? forcedId=null){ var id=forcedId??CurrentNodeId(); if(id==null)return; using var f=new SimpleComboForm("平仓",new FieldSpec("平仓价格","text"),new FieldSpec("平仓订单号","text")); if(f.ShowDialog(this)==DialogResult.OK){ _store.Exec("UPDATE position_nodes SET close_exit_price=$p,exchange_close_order_id=$o,close_time=datetime('now','localtime'),status='已平仓',floating_pnl=(($p-entry_price)*qty) WHERE id=$id",("$p",DbNum(f.Values[0])),("$o",f.Values[1]),("$id",id)); RefreshAll(); }}
    string[] MainCodeOptions(long? id=null){ var used=_store.Query("SELECT code FROM main_positions WHERE account_key=$a",("$a",AccountKey)).Rows.Cast<DataRow>().Select(r=>Convert.ToString(r[0])??"").ToHashSet(); return Enumerable.Range(1,50).Select(i=>$"M{i}").OrderBy(c=>used.Contains(c)?1:0).ThenBy(c=>int.Parse(c[1..])).ToArray(); }
    ...
    void EditNode(long? id, string? presetType){
        ...
        using var f=new NodeEditForm(row,_store.Config.DefaultFeeRatePct,presetType, NodeCodesFor(typeForCodes), id==null?NextNodeCode(typeForCodes):Convert.ToString(row?["code"]), AccountKey);
        if(f.ShowDialog(this)==DialogResult.OK){
            var v=f.V; var autoFee=v.Fee==0?Math.Abs(v.Qty*v.Entry*_store.Config.DefaultFeeRatePct/100m):v.Fee; var autoBe=v.Breakeven==0?Breakeven(v.Direction,v.Entry,_store.Config.DefaultFeeRatePct):v.Breakeven;
            try{
                if(id==null)_store.Exec("INSERT INTO position_nodes(account_key,main_code,code,node_type,direction,status,qty,entry_price,stop_loss,fee,funding,breakeven_price,cost_stop,trailing_take_profit,exchange_open_order_id,exchange_close_order_id,created_at,note) VALUES($a,$m,$c,$t,$d,$s,$q,$e,$cl,$f,$fu,$b,$cs,$tp,$oo,$co,datetime('now','localtime'),$n)",("$a",AccountKey),("$m",v.MainCode),("$c",v.Code),("$t",v.Type),("$d",v.Direction),("$s",v.Status),("$q",v.Qty),("$e",v.Entry),("$cl",v.Close),("$f",autoFee),("$fu",v.Funding),("$b",autoBe),("$cs",v.CostStop),("$tp",v.TrailingTp),("$oo",v.OpenOrder),("$co",v.CloseOrder),("$n",v.Note));
                else _store.Exec("UPDATE position_nodes SET main_code=$m,code=$c,node_type=$t,direction=$d,status=$s,qty=$q,entry_price=$e,stop_loss=$cl,fee=$f,funding=$fu,breakeven_price=$b,cost_stop=$cs,trailing_take_profit=$tp,exchange_open_order_id=$oo,exchange_close_order_id=$co,note=$n WHERE id=$id",("$m",v.MainCode),("$c",v.Code),("$t",v.Type),("$d",v.Direction),("$s",v.Status),("$q",v.Qty),("$e",v.Entry),("$cl",v.Close),("$f",autoFee),("$fu",v.Funding),("$b",autoBe),("$cs",v.CostStop),("$tp",v.TrailingTp),("$oo",v.OpenOrder),("$co",v.CloseOrder),("$n",v.Note),("$id",id));
                RefreshAll();
    ...
    void DeleteNode(){ var id=CurrentNodeId(); if(id==null)return; if(MessageBox.Show(this,"删除选中节点？","确认",MessageBoxButtons.YesNo)==DialogResult.Yes){ _store.Exec("DELETE FROM position_nodes WHERE id=$id",("$id",id)); RefreshAll(); } }
...
public record MainValue(string Code,string Direction,string Cycle,string Status,decimal Qty,decimal Price,decimal HardStop,decimal Margin,decimal Fee,decimal Breakeven,string OpenOrder,string Note);
public sealed class NodeEditForm:SimpleComboForm{ public NodeValue V=>new(Values[0],Values[1],Values[2],Values[3],Values[4],Dec(Values[5]),Dec(Values[6]),Dec(Values[7]),Dec(Values[8]),0,Dec(Values[9]),0,0,Values[10],Values[11],Values[12]); public NodeEditForm(DataRow? r,decimal feeRate,string? presetType=null,string[]? codeOptions=null,string? defaultCode=null,string accountKey="OKX_COIN"):base(r==null?($"新增{presetType ?? "节点"}"):"修改节点", new FieldSpec("主仓编号","combo",Enumerable.Range(1,10).Select(i=>$"M{i}").ToArray(),S(r,"main_code","M1")), new FieldSpec("节点编号","combo",codeOptions??CodesFor(presetType),S(r,"code",defaultCode??DefaultCode(presetType))), new FieldSpec("类型","combo",new[]{"加仓","对冲","减仓"},S(r,"node_type",presetType ?? "加仓")), new FieldSpec("方向","combo",new[]{"多","空"},S(r,"direction","多")), new FieldSpec("状态","combo",new[]{"已开仓","持有中","计划中","成本止损","移动止盈","已平仓","已取消","待归类"},S(r,"status","已开仓")), new FieldSpec(accountKey=="OKX_COIN"?"数量(BTC)":"数量(USDT)","text",Default:S(r,"qty","")), new FieldSpec("开仓价","text",Default:S(r,"entry_price","")), new FieldSpec("止损价","text",Default:S(r,"close_price","")), new FieldSpec("手续费(自动)","text",Default:S(r,"fee","")), new FieldSpec("盈亏平衡(自动)","text",Default:S(r,"breakeven_price","")), new FieldSpec("开仓订单号","text",Default:S(r,"exchange_open_order_id","")), new FieldSpec("平仓订单号","text",Default:S(r,"exchange_close_order_id","")), new FieldSpec("备注","text",Default:S(r,"note",""))){
        void Recalc(){
...
public sealed class ReduceEditForm:SimpleComboForm{ public ReduceValue V=>new(Values[0],Values[1],Values[2],Dec(Values[3]),Dec(Values[4]),Dec(Values[5]),Dec(Values[6]),Values[7],Values[8]); public ReduceEditForm(DataRow r,decimal feeRate,string code):base("减仓", new FieldSpec("编号","combo",Enumerable.Range(1,1000).Select(i=>$"R{i}").ToArray(),code), new FieldSpec("方向","combo",new[]{"多","空"},S(r,"direction","多")), new FieldSpec("状态","combo",new[]{"已平仓"},"已平仓"), new FieldSpec("减仓量","text",Default:""), new FieldSpec("开仓均价","text",Default:S(r,"entry_price","")), new FieldSpec("减仓价格","text",Default:""), new FieldSpec("手续费","text",Default:""), new FieldSpec("平仓订单号","text",Default:""), new FieldSpec("备注","text",Default:"")){ void Recalc(){ if(decimal.TryParse(_inputs[3].Text,NumberStyles.Any,CultureInfo.InvariantCulture,out var q)&&decimal.TryParse(_inputs[5].Text,NumberStyles.Any,CultureInfo.InvariantCulture,out var px)){ _inputs[6].Text=(Math.Abs(q*px*feeRate/100m)).ToString("0.########",CultureInfo.InvariantCulture);} } _inputs[3].TextChanged+=(_,_)=>Recalc(); _inputs[5].TextChanged+=(_,_)=>Recalc(); } static string S(DataRow? r,string c,string d)=>r==null?d:Convert.ToString(r[c])??d; static decimal Dec(string s)=>decimal.TryParse(s,NumberStyles.Any,CultureInfo.InvariantCulture,out var d)?d:0m; }
public record ReduceValue(string Code,string Direction,string Status,decimal Qty,decimal Entry,decimal Close,decimal Fee,string CloseOrder,string Note);
