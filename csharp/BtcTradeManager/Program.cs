...
public sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BTC交易管理系统 v0.7.0"; Font=new Font("Microsoft YaHei",9); StartPosition=FormStartPosition.CenterScreen; MinimumSize=new Size(1120,680); Size=new Size(1360,820); LoadUi(); BuildUi(); RefreshAll();
    ...
    static DataGridView Grid(){
        ...
        g.ColumnDividerDoubleClick += (sender,e)=>{ var dg=(DataGridView)sender!; if(e.ColumnIndex>=0){ var col=dg.Columns[e.ColumnIndex]; var w=TextRenderer.MeasureText(col.HeaderText??col.Name,dg.ColumnHeadersDefaultCellStyle.Font??dg.Font).Width+28; col.Width=Math.Max(48,Math.Min(220,w)); } }; g.MouseClick += (sender,e)=>{ var dg=(DataGridView)sender!; var hit=dg.HitTest(e.X,e.Y); if(hit.Type==DataGridViewHitTestType.None || hit.RowIndex<0){ try{ dg.ClearSelection(); dg.CurrentCell=null; }catch{} } };
        return g;
    }
    ...
    void NameGrids(){ _summary.Name="总控"; _summary.Font=new Font("Microsoft YaHei",13,FontStyle.Bold); _summary.DefaultCellStyle.Font=new Font("Microsoft YaHei",13,FontStyle.Bold); _summary.ColumnHeadersDefaultCellStyle.Font=new Font("Microsoft YaHei",13,FontStyle.Bold); _summary.RowTemplate.Height=46; _summary.DefaultCellStyle.WrapMode=DataGridViewTriState.True; _summary.AutoSizeColumnsMode=DataGridViewAutoSizeColumnsMode.Fill; _summary.BackgroundColor=Color.White; _mainGrid.Name="主仓M"; _addGrid.Name="加仓A"; _hedgeGrid.Name="对冲H"; _reduceGrid.Name="减仓R"; _closedGrid.Name="已平仓"; _orderGrid.Name="订单"; _riskGrid.Name="风险"; _scenarioGrid.Name="情景"; }
    ...
    void HideId(DataGridView g){
        ...
        if(g.Columns.Contains("id")) g.Columns["id"].Visible=false; foreach(DataGridViewColumn c in g.Columns){ c.DefaultCellStyle.Alignment=DataGridViewContentAlignment.MiddleCenter; c.HeaderCell.Style.Alignment=DataGridViewContentAlignment.MiddleCenter; } if(!ReferenceEquals(g,_summary)) ApplyGridWidths(g); else { try{ if(g.Columns.Contains("项目")) g.Columns["项目"].Width=190; if(g.Columns.Contains("数值")) g.Columns["数值"].Width=290; if(g.Columns.Contains("说明")) g.Columns["说明"].AutoSizeMode=DataGridViewAutoSizeColumnMode.Fill; g.AutoResizeRows(DataGridViewAutoSizeRowsMode.AllCells); }catch{} }
