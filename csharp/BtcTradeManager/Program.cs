...
public sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BTC交易管理系统 v0.6.4"; Font=new Font("Microsoft YaHei",9); StartPosition=FormStartPosition.CenterScreen; MinimumSize=new Size(1120,680); Size=new Size(1360,820); LoadUi(); BuildUi(); RefreshAll();
    ...
    static DataGridView Grid(){
        var g=new DataGridView{Dock=DockStyle.Fill,Font=new Font("Microsoft YaHei",9),DefaultCellStyle=new DataGridViewCellStyle{Font=new Font("Microsoft YaHei",9)},ColumnHeadersDefaultCellStyle=new DataGridViewCellStyle{Font=new Font("Microsoft YaHei",9,FontStyle.Bold)},ReadOnly=true,AllowUserToAddRows=false,AllowUserToDeleteRows=false,RowHeadersVisible=false,AutoSizeColumnsMode=DataGridViewAutoSizeColumnsMode.None,SelectionMode=DataGridViewSelectionMode.FullRowSelect,MultiSelect=false,AutoGenerateColumns=true};
        g.DataError += (_,e)=>{ e.ThrowException=false; e.Cancel=true; };
        g.ColumnDividerDoubleClick += (sender,e)=>{ var dg=(DataGridView)sender!; if(e.ColumnIndex>=0){ dg.AutoResizeColumn(e.ColumnIndex,DataGridViewAutoSizeColumnMode.AllCells); } };
        return g;
    }
    ...
    void BuildUi()
    {
        var root=new TableLayoutPanel{Dock=DockStyle.Fill,RowCount=2,ColumnCount=1}; root.RowStyles.Add(new RowStyle(SizeType.Absolute,50)); root.RowStyles.Add(new RowStyle(SizeType.Percent,100)); Controls.Add(root); Controls.Add(_status); NameGrids(); _tree.ImageList=BuildStatusImages(); _tabs.DrawMode=TabDrawMode.OwnerDrawFixed; _tabs.Padding=new Point(18,4); _tabs.DrawItem+=DrawTab;
    ...
    void NameGrids(){ _summary.Name="总控"; _mainGrid.Name="主仓M"; _addGrid.Name="加仓A"; _hedgeGrid.Name="对冲H"; _reduceGrid.Name="减仓R"; _closedGrid.Name="已平仓"; _orderGrid.Name="订单"; _riskGrid.Name="风险"; _scenarioGrid.Name="情景"; }
    ...
    string StatusKey(string? s)=>s=="已开仓"||s=="持有中"?"red":s=="成本止损"?"yellow":s=="移动止盈"?"green":"gray";
    ...
    void CloseMain(long? forcedId=null){ var id=forcedId??RowId(_mainGrid); if(id==null)return; using var f=new SimpleComboForm("平仓",new FieldSpec("平仓价格","text"),new FieldSpec("平仓时间","text",Default:DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")),new FieldSpec("平仓订单号","text")); if(f.ShowDialog(this)==DialogResult.OK){ _store.Exec("UPDATE main_positions SET close_price=$p,exchange_close_order_id=$o,close_time=$tm,status='已平仓' WHERE id=$id",("$p",DbNum(f.Values[0])),("$o",f.Values.Length>2?f.Values[2]:f.Values[1]),("$tm",f.Values.Length>2?f.Values[1]:DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")),("$id",id)); RefreshAll(); }}
    void CloseNode(long? forcedId=null){ var id=forcedId??CurrentNodeId(); if(id==null)return; using var f=new SimpleComboForm("平仓",new FieldSpec("平仓价格","text"),new FieldSpec("平仓时间","text",Default:DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")),new FieldSpec("平仓订单号","text")); if(f.ShowDialog(this)==DialogResult.OK){ _store.Exec("UPDATE position_nodes SET close_exit_price=$p,exchange_close_order_id=$o,close_time=$tm,status='已平仓',floating_pnl=(CASE WHEN direction='空' THEN (entry_price-$p)*qty ELSE ($p-entry_price)*qty END)-fee-ABS(qty*$p*0.0005) WHERE id=$id",("$p",DbNum(f.Values[0])),("$o",f.Values.Length>2?f.Values[2]:f.Values[1]),("$tm",f.Values.Length>2?f.Values[1]:DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")),("$id",id)); RefreshAll(); }}
...
public sealed class ReduceEditForm:SimpleComboForm{ public ReduceValue V=>new(Values[0],Values[1],Values[2],Dec(Values[3]),Dec(Values[4]),Dec(Values[5]),Dec(Values[6]),Values[7],Values[8],Values[9],Values[10]); public ReduceEditForm(DataRow r,decimal feeRate,string code,string sourceKind="node"):base("减仓", new FieldSpec("编号","combo",Enumerable.Range(1,1000).Select(i=>$"R{i}").ToArray(),code), new FieldSpec("减仓订单编号","text",Default:S(r,sourceKind=="main"?"code":"code","")), new FieldSpec("状态","combo",new[]{"已减仓"},"已减仓"), new FieldSpec("减仓量","text",Default:""), new FieldSpec("开仓均价","text",Default:S(r,sourceKind=="main"?"avg_price":"entry_price","")), new FieldSpec("减仓价格","text",Default:""), new FieldSpec("手续费","text",Default:""), new FieldSpec("开仓订单号","text",Default:S(r,"exchange_open_order_id","")), new FieldSpec("开仓时间","text",Default:S(r,"created_at","")), new FieldSpec("平仓订单号","text",Default:""), new FieldSpec("备注","text",Default:"")){ void Recalc(){ if(decimal.TryParse(_inputs[3].Text,NumberStyles.Any,CultureInfo.InvariantCulture,out var q)&&decimal.TryParse(_inputs[5].Text,NumberStyles.Any,CultureInfo.InvariantCulture,out var px)){ _inputs[6].Text=(Math.Abs(q*px*feeRate/100m)).ToString("0.########",CultureInfo.InvariantCulture);} } _inputs[3].TextChanged+=(_,_)=>Recalc(); _inputs[5].TextChanged+=(_,_)=>Recalc(); } static string S(DataRow? r,string c,string d)=>r==null||!r.Table.Columns.Contains(c)?d:Convert.ToString(r[c])??d; static decimal Dec(string s)=>decimal.TryParse(s,NumberStyles.Any,CultureInfo.InvariantCulture,out var d)?d:0m; }
public record ReduceValue(string Code,string SourceCode,string Status,decimal Qty,decimal Entry,decimal Close,decimal Fee,string OpenOrder,string OpenTime,string CloseOrder,string Note);
