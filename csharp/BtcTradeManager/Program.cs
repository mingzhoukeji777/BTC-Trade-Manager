...
public sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BTC交易管理系统 v0.6.9"; Font=new Font("Microsoft YaHei",9); StartPosition=FormStartPosition.CenterScreen; MinimumSize=new Size(1120,680); Size=new Size(1360,820); LoadUi(); BuildUi(); RefreshAll();
    ...
    void EditClosed(long? id){ if(id==null)return; var code=Convert.ToString(_closedGrid.CurrentRow?.Cells["编号"].Value)??""; var isNode=code.Contains('-'); var rowType=Convert.ToString(_closedGrid.CurrentRow?.Cells["类型"].Value)??(isNode?"节点":"主仓"); using var f=new SimpleComboForm("修改已平仓", new FieldSpec("类型","text",Default:rowType), new FieldSpec("平仓类型","combo",new[]{"止损","成本止损","移动止盈","手动平仓"}, Convert.ToString(_closedGrid.CurrentRow?.Cells["平仓类型"].Value)??"手动平仓"), new FieldSpec("平仓时间","text",Default:Convert.ToString(_closedGrid.CurrentRow?.Cells["平仓时间"].Value)??""), new FieldSpec("平仓订单号","text",Default:Convert.ToString(_closedGrid.CurrentRow?.Cells["平仓订单号"].Value)??""), new FieldSpec("备注","text",Default:Convert.ToString(_closedGrid.CurrentRow?.Cells["备注"].Value)??"")); if(f.ShowDialog(this)==DialogResult.OK){ if(isNode) _store.Exec("UPDATE position_nodes SET close_type=$t,close_time=$tm,exchange_close_order_id=$o,note=$n WHERE id=$id",("$t",f.Values[1]),("$tm",f.Values[2]),("$o",f.Values[3]),("$n",f.Values[4]),("$id",id)); else _store.Exec("UPDATE main_positions SET close_type=$t,close_time=$tm,exchange_close_order_id=$o,note=$n WHERE id=$id",("$t",f.Values[1]),("$tm",f.Values[2]),("$o",f.Values[3]),("$n",f.Values[4]),("$id",id)); RefreshAll(); } }
