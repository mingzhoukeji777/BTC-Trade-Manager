...
public sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BTC交易管理系统 v0.6.8"; Font=new Font("Microsoft YaHei",9); StartPosition=FormStartPosition.CenterScreen; MinimumSize=new Size(1120,680); Size=new Size(1360,820); LoadUi(); BuildUi(); RefreshAll();
    ...
    static DataGridView Grid(){
        var g=new DataGridView{Dock=DockStyle.Fill,Font=new Font("Microsoft YaHei",9),DefaultCellStyle=new DataGridViewCellStyle{Font=new Font("Microsoft YaHei",9)},ColumnHeadersDefaultCellStyle=new DataGridViewCellStyle{Font=new Font("Microsoft YaHei",9,FontStyle.Bold)},ReadOnly=true,AllowUserToAddRows=false,AllowUserToDeleteRows=false,RowHeadersVisible=false,AutoSizeColumnsMode=DataGridViewAutoSizeColumnsMode.None,SelectionMode=DataGridViewSelectionMode.FullRowSelect,MultiSelect=false,AutoGenerateColumns=true,BackgroundColor=Color.White,GridColor=Color.LightGray};
        ...
        g.ColumnDividerDoubleClick += (sender,e)=>{ var dg=(DataGridView)sender!; if(e.ColumnIndex>=0){ dg.AutoResizeColumn(e.ColumnIndex,DataGridViewAutoSizeColumnMode.DisplayedCells); if(dg.Columns[e.ColumnIndex].Width>260) dg.Columns[e.ColumnIndex].Width=260; } }; g.MouseDoubleClick += (sender,e)=>{ var dg=(DataGridView)sender!; var hit=dg.HitTest(e.X,e.Y); if(hit.Type==DataGridViewHitTestType.None || hit.RowIndex<0){ try{ dg.ClearSelection(); dg.CurrentCell=null; }catch{} } };
        return g;
    }
    ...
    void NameGrids(){ _summary.Name="总控"; _summary.Font=new Font("Microsoft YaHei",11,FontStyle.Bold); _summary.DefaultCellStyle.Font=new Font("Microsoft YaHei",11); _summary.ColumnHeadersDefaultCellStyle.Font=new Font("Microsoft YaHei",11,FontStyle.Bold); _summary.RowTemplate.Height=34; _summary.AutoSizeColumnsMode=DataGridViewAutoSizeColumnsMode.Fill; _summary.BackgroundColor=Color.White; _mainGrid.Name="主仓M"; _addGrid.Name="加仓A"; _hedgeGrid.Name="对冲H"; _reduceGrid.Name="减仓R"; _closedGrid.Name="已平仓"; _orderGrid.Name="订单"; _riskGrid.Name="风险"; _scenarioGrid.Name="情景"; }
    ...
    void LoadNodeGrid(){
        ...
        var closed="SELECT id,编号,方向,状态,平仓类型,持仓量,盈亏,盈亏金额,开仓均价,平仓价格,手续费,开仓时间,平仓时间,开仓订单号,平仓订单号,备注 FROM (SELECT id,code AS 编号,direction AS 方向,status AS 状态,close_type AS 平仓类型,base_qty AS 持仓量,CASE WHEN floating_pnl>=0 THEN '盈' ELSE '亏' END AS 盈亏,floating_pnl AS 盈亏金额,avg_price AS 开仓均价,close_price AS 平仓价格,fee AS 手续费,created_at AS 开仓时间,close_time AS 平仓时间,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注,close_time AS sort_time FROM main_positions WHERE account_key=$a AND status='已平仓' UNION ALL SELECT id,main_code||'-'||code AS 编号,direction AS 方向,status AS 状态,close_type AS 平仓类型,qty AS 持仓量,CASE WHEN floating_pnl>=0 THEN '盈' ELSE '亏' END AS 盈亏,floating_pnl AS 盈亏金额,entry_price AS 开仓均价,close_exit_price AS 平仓价格,fee AS 手续费,created_at AS 开仓时间,close_time AS 平仓时间,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注,close_time AS sort_time FROM position_nodes WHERE account_key=$a AND status='已平仓') ORDER BY sort_time DESC,id DESC";
    ...
    void EditClosed(long? id){ if(id==null)return; var code=Convert.ToString(_closedGrid.CurrentRow?.Cells["编号"].Value)??""; var isNode=code.Contains('-'); using var f=new SimpleComboForm("修改已平仓", new FieldSpec("平仓类型","combo",new[]{"止损","成本止损","移动止盈","手动平仓"}, Convert.ToString(_closedGrid.CurrentRow?.Cells["平仓类型"].Value)??"手动平仓"), new FieldSpec("平仓时间","text",Default:Convert.ToString(_closedGrid.CurrentRow?.Cells["平仓时间"].Value)??""), new FieldSpec("平仓订单号","text",Default:Convert.ToString(_closedGrid.CurrentRow?.Cells["平仓订单号"].Value)??""), new FieldSpec("备注","text",Default:Convert.ToString(_closedGrid.CurrentRow?.Cells["备注"].Value)??"")); if(f.ShowDialog(this)==DialogResult.OK){ if(isNode) _store.Exec("UPDATE position_nodes SET close_type=$t,close_time=$tm,exchange_close_order_id=$o,note=$n WHERE id=$id",("$t",f.Values[0]),("$tm",f.Values[1]),("$o",f.Values[2]),("$n",f.Values[3]),("$id",id)); else _store.Exec("UPDATE main_positions SET close_type=$t,close_time=$tm,exchange_close_order_id=$o,note=$n WHERE id=$id",("$t",f.Values[0]),("$tm",f.Values[1]),("$o",f.Values[2]),("$n",f.Values[3]),("$id",id)); RefreshAll(); } }
