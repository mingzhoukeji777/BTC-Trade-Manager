...
public sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BTC交易管理系统 v0.6.2"; Font=new Font("Microsoft YaHei UI",9); StartPosition=FormStartPosition.CenterScreen; MinimumSize=new Size(1120,680); Size=new Size(1360,820); LoadUi(); BuildUi(); RefreshAll();
    ...
    void LoadNodeGrid(){
        ...
        _reduceGrid.DataSource=_store.Query(cols+" AND node_type='减仓' AND status<>'已平仓' ORDER BY id",("$a",AccountKey)); HideId(_reduceGrid);
        _closedGrid.DataSource=_store.Query(cols+" AND status='已平仓' ORDER BY id DESC",("$a",AccountKey)); HideId(_closedGrid);
    }
    ...
    long? CurrentNodeId(){ if(_tabs.SelectedTab?.Text=="加仓") return RowId(_addGrid); if(_tabs.SelectedTab?.Text=="对冲") return RowId(_hedgeGrid); if(_tabs.SelectedTab?.Text=="减仓") return RowId(_reduceGrid); if(_tabs.SelectedTab?.Text=="已平仓") return RowId(_closedGrid); return RowId(_nodeGrid); }
