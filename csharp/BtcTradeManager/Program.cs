...
public sealed class MainForm:Form
{
    ...
    readonly DataGridView _summary = Grid(); readonly TreeView _tree = new(){Dock=DockStyle.Fill,HideSelection=false,Font=new Font("Microsoft YaHei UI",10)};
    readonly DataGridView _mainGrid=Grid(), _nodeGrid=Grid(), _addGrid=Grid(), _hedgeGrid=Grid(), _reduceGrid=Grid(), _orderGrid=Grid(), _riskGrid=Grid(), _scenarioGrid=Grid(); bool _loadingUi;
    ...
    void BuildUi()
    {
        ...
        AddButton(top,"刷新",RefreshAll); AddButton(top,"新增主仓",()=>EditMain(null)); AddButton(top,"加仓",()=>EditNodePreset("加仓")); AddButton(top,"对冲",()=>EditNodePreset("对冲")); AddButton(top,"减仓",()=>EditNodePreset("减仓")); AddButton(top,"归类订单",()=>EditOrder(null)); AddButton(top,"情景模拟",RunScenario); AddButton(top,"设置",OpenSettings);
        root.Controls.Add(_tabs,0,1);
        _tabs.TabPages.Add(Page("总控", _summary)); _tabs.TabPages.Add(Page("主仓管理", WithMenu(_mainGrid, MainMenu()))); _tabs.TabPages.Add(Page("主仓树", WithMenu(_tree, TreeMenu()))); _tabs.TabPages.Add(Page("加仓", WithMenu(_addGrid, NodeMenu()))); _tabs.TabPages.Add(Page("对冲", WithMenu(_hedgeGrid, NodeMenu()))); _tabs.TabPages.Add(Page("减仓", WithMenu(_reduceGrid, NodeMenu()))); _tabs.TabPages.Add(Page("订单映射/待归类", WithMenu(_orderGrid, OrderMenu()))); _tabs.TabPages.Add(Page("风险总控", _riskGrid)); _tabs.TabPages.Add(Page("情景模拟", _scenarioGrid));
        _mainGrid.CellDoubleClick += (_,e)=>{ if(e.RowIndex>=0) EditMain(RowId(_mainGrid)); }; _nodeGrid.CellDoubleClick += (_,e)=>{ if(e.RowIndex>=0) EditNode(RowId(_nodeGrid)); }; _addGrid.CellDoubleClick += (_,e)=>{ if(e.RowIndex>=0) EditNode(RowId(_addGrid)); }; _hedgeGrid.CellDoubleClick += (_,e)=>{ if(e.RowIndex>=0) EditNode(RowId(_hedgeGrid)); }; _reduceGrid.CellDoubleClick += (_,e)=>{ if(e.RowIndex>=0) EditNode(RowId(_reduceGrid)); }; _orderGrid.CellDoubleClick += (_,e)=>{ if(e.RowIndex>=0) EditOrder(RowId(_orderGrid)); };
    }
    ...
    ContextMenuStrip NodeMenu(){ var m=new ContextMenuStrip(); m.Items.Add("修改",null,(_,_)=>EditNode(CurrentNodeId())); m.Items.Add("删除",null,(_,_)=>DeleteNode()); m.Items.Add("移动止盈",null,(_,_)=>SetProtection("移动止盈")); m.Items.Add("成本止损",null,(_,_)=>SetProtection("成本止损")); m.Items.Add("平仓",null,(_,_)=>CloseNode()); return m; }
    ...
    void LoadMainGrid(){ _mainGrid.DataSource=_store.Query("SELECT id,code AS 编号,direction AS 方向,cycle_name AS 周期,status AS 状态,base_qty AS 持仓量,avg_price AS 开仓均价,breakeven_price AS 盈亏平衡价,liq_price AS 强平价格,floating_pnl AS 浮动收益,maintenance_margin_rate AS 维持保证金率,margin AS 保证金,hard_stop AS 止损价格,cost_stop AS 成本止损,trailing_take_profit AS 移动止盈,close_price AS 平仓价格,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注 FROM main_positions WHERE account_key=$a ORDER BY id",("$a",AccountKey)); HideId(_mainGrid); }
    void LoadNodeGrid(){
        var cols="SELECT id,code AS 编号,direction AS 方向,'' AS 周期,status AS 状态,qty AS 持仓量,entry_price AS 开仓均价,breakeven_price AS 盈亏平衡价,liq_price AS 强平价格,floating_pnl AS 浮动收益,maintenance_margin_rate AS 维持保证金率,0 AS 保证金,COALESCE(stop_loss,close_price) AS 止损价格,cost_stop AS 成本止损,trailing_take_profit AS 移动止盈,close_exit_price AS 平仓价格,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注 FROM position_nodes WHERE account_key=$a";
        _nodeGrid.DataSource=_store.Query(cols+" ORDER BY id",("$a",AccountKey)); HideId(_nodeGrid);
        _addGrid.DataSource=_store.Query(cols+" AND node_type='加仓' ORDER BY id",("$a",AccountKey)); HideId(_addGrid);
        _hedgeGrid.DataSource=_store.Query(cols+" AND node_type='对冲' ORDER BY id",("$a",AccountKey)); HideId(_hedgeGrid);
    ...
    long? CurrentNodeId(){ if(_tabs.SelectedTab?.Text=="加仓") return RowId(_addGrid); if(_tabs.SelectedTab?.Text=="对冲") return RowId(_hedgeGrid); if(_tabs.SelectedTab?.Text=="减仓") return RowId(_reduceGrid); return RowId(_nodeGrid); }
...
public sealed class MainEditForm:SimpleComboForm{ public MainValue V=>new(Values[0],Values[1],Values[2],Values[3],Dec(Values[4]),Dec(Values[5]),Dec(Values[6]),Dec(Values[7]),Dec(Values[8]),Dec(Values[9]),Values[10],Values[11]); public MainEditForm(DataRow? r,string[] codeOptions):base(r==null?"新增主仓":"修改主仓", new FieldSpec("编号","combo",codeOptions,S(r,"code",codeOptions.FirstOrDefault()??"M1")), new FieldSpec("方向","combo",new[]{"多","空"},S(r,"direction","多")), new FieldSpec("周期","combo",new[]{"牛市周期","熊市周期"},S(r,"cycle_name","牛市周期")), new FieldSpec("状态","combo",new[]{"规划中","持仓中","已完成","已取消"},S(r,"status","规划中")), new FieldSpec("主仓数量(BTC/张)","text",Default:S(r,"base_qty","")), new FieldSpec("主仓均价(USDT/USD)","text",Default:S(r,"avg_price","")), new FieldSpec("硬止损价(USDT/USD)","text",Default:S(r,"hard_stop","")), new FieldSpec("保证金(USDT/BTC)","text",Default:S(r,"margin","")), new FieldSpec("手续费(USDT/BTC)","text",Default:S(r,"fee","")), new FieldSpec("盈亏平衡价","text",Default:S(r,"breakeven_price","")), new FieldSpec("开仓订单号","text",Default:S(r,"exchange_open_order_id","")), new FieldSpec("备注","text",Default:S(r,"note",""))){} static string[] CodesFor(string? t){ string prefix=t switch{ "加仓"=>"A", "减仓"=>"R", "对冲"=>"H", "移动止盈"=>"TP", "成本止损"=>"SL", _=>"A"}; return Enumerable.Range(1,100).Select(i=>$"{prefix}{i}").ToArray(); } static string DefaultCode(string? t)=>CodesFor(t).First(); static string S(DataRow? r,string c,string d)=>r==null?d:Convert.ToString(r[c])??d; static decimal Dec(string s)=>decimal.TryParse(s,NumberStyles.Any,CultureInfo.InvariantCulture,out var d)?d:0m; }
...
public sealed class NodeEditForm:SimpleComboForm{ public NodeValue V=>new(Values[0],Values[1],Values[2],Values[3],Values[4],Dec(Values[5]),Dec(Values[6]),Dec(Values[7]),Dec(Values[8]),0,Dec(Values[9]),0,0,Values[10],Values[11],Values[12]); public NodeEditForm(DataRow? r,decimal feeRate,string? presetType=null,string[]? codeOptions=null,string? defaultCode=null):base(r==null?($"新增{presetType ?? "节点"}"):"修改节点", new FieldSpec("主仓编号","combo",Enumerable.Range(1,10).Select(i=>$"M{i}").ToArray(),S(r,"main_code","M1")), new FieldSpec("节点编号","combo",codeOptions??CodesFor(presetType),S(r,"code",defaultCode??DefaultCode(presetType))), new FieldSpec("类型","combo",new[]{"加仓","对冲","减仓"},S(r,"node_type",presetType ?? "加仓")), new FieldSpec("方向","combo",new[]{"多","空"},S(r,"direction","多")), new FieldSpec("状态","combo",new[]{"计划中","已开仓","已平仓","已取消","待归类"},S(r,"status","计划中")), new FieldSpec("数量","text",Default:S(r,"qty","")), new FieldSpec("开仓价","text",Default:S(r,"entry_price","")), new FieldSpec("止损价","text",Default:S(r,"close_price","")), new FieldSpec("手续费(自动)","text",Default:S(r,"fee","")), new FieldSpec("盈亏平衡(自动)","text",Default:S(r,"breakeven_price","")), new FieldSpec("开仓订单号","text",Default:S(r,"exchange_open_order_id","")), new FieldSpec("平仓订单号","text",Default:S(r,"exchange_close_order_id","")), new FieldSpec("备注","text",Default:S(r,"note",""))){
        ...
        Recalc();
    } static string[] CodesFor(string? t){ string prefix=t switch{ "加仓"=>"A", "减仓"=>"R", "对冲"=>"H", "移动止盈"=>"TP", "成本止损"=>"SL", _=>"A"}; return Enumerable.Range(1,100).Select(i=>$"{prefix}{i}").ToArray(); } static string DefaultCode(string? t)=>CodesFor(t).First(); static string S(DataRow? r,string c,string d)=>r==null?d:Convert.ToString(r[c])??d; static decimal Dec(string s)=>decimal.TryParse(s,NumberStyles.Any,CultureInfo.InvariantCulture,out var d)?d:0m; }
