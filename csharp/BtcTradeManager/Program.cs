...
public sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BTC交易管理系统 v0.5"; Font=new Font("Microsoft YaHei UI",9); StartPosition=FormStartPosition.CenterScreen; MinimumSize=new Size(1120,680); Size=new Size(1360,820); LoadUi(); BuildUi(); RefreshAll();
    ...
    ContextMenuStrip NodeMenu(){ var m=new ContextMenuStrip(); m.Items.Add("新增加仓",null,(_,_)=>EditNodePreset("加仓")); m.Items.Add("新增对冲",null,(_,_)=>EditNodePreset("对冲")); m.Items.Add("新增减仓",null,(_,_)=>EditNodePreset("减仓")); m.Items.Add("修改节点",null,(_,_)=>EditNode(CurrentNodeId())); m.Items.Add("删除节点",null,(_,_)=>DeleteNode()); m.Items.Add("添加成本止损",null,(_,_)=>SetProtection("成本止损")); m.Items.Add("添加移动止盈",null,(_,_)=>SetProtection("移动止盈")); return m; }
    ...
    ContextMenuStrip TreeMenu(){ var m=new ContextMenuStrip(); m.Items.Add("新增主仓",null,(_,_)=>EditMain(null)); m.Items.Add("新增加仓",null,(_,_)=>EditNodePreset("加仓")); m.Items.Add("新增对冲",null,(_,_)=>EditNodePreset("对冲")); m.Items.Add("新增减仓",null,(_,_)=>EditNodePreset("减仓")); m.Items.Add("修改选中",null,(_,_)=>EditTreeSelected()); m.Items.Add("删除选中",null,(_,_)=>DeleteTreeSelected()); return m; }
    ...
    void LoadMainGrid(){ _mainGrid.DataSource=_store.Query("SELECT id,code AS 编号,direction AS 方向,cycle_name AS 周期,status AS 状态,base_qty AS 主仓数量单位,avg_price AS 主仓均价单位,hard_stop AS 硬止损价,margin AS 保证金单位,fee AS 手续费,breakeven_price AS 盈亏平衡,exchange_open_order_id AS 开仓订单号,created_at AS 创建时间,note AS 备注 FROM main_positions WHERE account_key=$a ORDER BY id",("$a",AccountKey)); HideId(_mainGrid); }
    void LoadNodeGrid(){
        var cols="SELECT id,code AS 编号,node_type AS 类型,direction AS 方向,status AS 状态,qty AS 数量,entry_price AS 开仓价,close_price AS 止损价,fee AS 手续费,breakeven_price AS 盈亏平衡,exchange_open_order_id AS 开仓订单号,exchange_close_order_id AS 平仓订单号,note AS 备注 FROM position_nodes WHERE account_key=$a";
    ...
    void EditMain(long? id){ var row=id==null?null:_store.Query("SELECT * FROM main_positions WHERE id=$id",("$id",id)).Rows.Cast<DataRow>().FirstOrDefault(); using var f=new MainEditForm(row, MainCodeOptions(id)); if(f.ShowDialog(this)==DialogResult.OK){ var v=f.V; if(id==null)_store.Exec("INSERT OR IGNORE INTO main_positions(account_key,code,direction,cycle_name,status,base_qty,avg_price,hard_stop,margin,fee,exchange_open_order_id,created_at,note) VALUES($a,$c,$d,$cy,$s,$q,$p,$hs,$m,$f,$oo,datetime('now','localtime'),$n)",("$a",AccountKey),("$c",v.Code),("$d",v.Direction),("$cy",v.Cycle),("$s",v.Status),("$q",v.Qty),("$p",v.Price),("$hs",v.HardStop),("$m",v.Margin),("$f",v.Fee),("$oo",v.OpenOrder),("$n",v.Note)); else _store.Exec("UPDATE main_positions SET code=$c,direction=$d,cycle_name=$cy,status=$s,base_qty=$q,avg_price=$p,hard_stop=$hs,margin=$m,fee=$f,exchange_open_order_id=$oo,note=$n WHERE id=$id",("$c",v.Code),("$d",v.Direction),("$cy",v.Cycle),("$s",v.Status),("$q",v.Qty),("$p",v.Price),("$hs",v.HardStop),("$m",v.Margin),("$f",v.Fee),("$oo",v.OpenOrder),("$n",v.Note),("$id",id)); RefreshAll(); } }
