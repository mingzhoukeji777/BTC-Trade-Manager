...
public sealed class MainForm:Form
{
    ...
    public MainForm()
    {
        Text="BTC交易管理系统 v0.4"; Font=new Font("Microsoft YaHei UI",9); StartPosition=FormStartPosition.CenterScreen; MinimumSize=new Size(1120,680); Size=new Size(1360,820); LoadUi(); BuildUi(); RefreshAll();
    ...
    void BuildUi()
    {
        ...
        AddButton(top,"刷新",RefreshAll); AddButton(top,"新增主仓",()=>EditMain(null)); AddButton(top,"加仓",()=>EditNodePreset("加仓")); AddButton(top,"减仓",()=>EditNodePreset("减仓")); AddButton(top,"对冲",()=>EditNodePreset("对冲")); AddButton(top,"移动止盈",()=>EditNodePreset("移动止盈")); AddButton(top,"成本止损",()=>EditNodePreset("成本止损")); AddButton(top,"归类订单",()=>EditOrder(null)); AddButton(top,"情景模拟",RunScenario); AddButton(top,"设置",OpenSettings);
    ...
    ContextMenuStrip NodeMenu(){ var m=new ContextMenuStrip(); m.Items.Add("新增加仓",null,(_,_)=>EditNodePreset("加仓")); m.Items.Add("新增减仓",null,(_,_)=>EditNodePreset("减仓")); m.Items.Add("新增对冲",null,(_,_)=>EditNodePreset("对冲")); m.Items.Add("新增移动止盈",null,(_,_)=>EditNodePreset("移动止盈")); m.Items.Add("新增成本止损",null,(_,_)=>EditNodePreset("成本止损")); m.Items.Add("修改节点",null,(_,_)=>EditNode(RowId(_nodeGrid))); m.Items.Add("删除节点",null,(_,_)=>DeleteNode()); return m; }
    ...
    ContextMenuStrip TreeMenu(){ var m=new ContextMenuStrip(); m.Items.Add("新增主仓",null,(_,_)=>EditMain(null)); m.Items.Add("新增加仓",null,(_,_)=>EditNodePreset("加仓")); m.Items.Add("新增减仓",null,(_,_)=>EditNodePreset("减仓")); m.Items.Add("新增对冲",null,(_,_)=>EditNodePreset("对冲")); m.Items.Add("新增移动止盈",null,(_,_)=>EditNodePreset("移动止盈")); m.Items.Add("新增成本止损",null,(_,_)=>EditNodePreset("成本止损")); m.Items.Add("修改选中",null,(_,_)=>EditTreeSelected()); m.Items.Add("删除选中",null,(_,_)=>DeleteTreeSelected()); return m; }
...
public sealed class MainEditForm:SimpleComboForm{ public MainValue V=>new(Values[0],Values[1],Values[2],Values[3],Dec(Values[4]),Dec(Values[5]),Dec(Values[6]),Values[7]); public MainEditForm(DataRow? r):base(r==null?"新增主仓":"修改主仓", new FieldSpec("编号","combo",Enumerable.Range(1,10).Select(i=>$"M{i}").ToArray(),S(r,"code","M1")), new FieldSpec("方向","combo",new[]{"多","空"},S(r,"direction","多")), new FieldSpec("周期","combo",new[]{"牛市周期","熊市周期"},S(r,"cycle_name","牛市周期")), new FieldSpec("状态","combo",new[]{"规划中","持仓中","已完成","已取消"},S(r,"status","规划中")), new FieldSpec("主仓数量","text",Default:S(r,"base_qty","")), new FieldSpec("主仓均价","text",Default:S(r,"avg_price","")), new FieldSpec("保证金","text",Default:S(r,"margin","")), new FieldSpec("备注","text",Default:S(r,"note",""))){} static string[] CodesFor(string? t)=>t switch{ "加仓"=>new[]{"A1","A2","A3","A4","A5"}, "减仓"=>new[]{"R1","R2","R3","R4","R5"}, "对冲"=>new[]{"H1","H2","H3","H4","H5"}, "移动止盈"=>new[]{"TP1","TP2","TP3","TP4","TP5"}, "成本止损"=>new[]{"SL1","SL2","SL3","SL4","SL5"}, _=>new[]{"A1","A2","A3","A4","A5","H1","H2","H3","TP1","TP2","TP3","SL1","SL2","R1","R2"}}; static string DefaultCode(string? t)=>CodesFor(t).First(); static string S(DataRow? r,string c,string d)=>r==null?d:Convert.ToString(r[c])??d; static decimal Dec(string s)=>decimal.TryParse(s,NumberStyles.Any,CultureInfo.InvariantCulture,out var d)?d:0m; }
...
public sealed class NodeEditForm:SimpleComboForm{ public NodeValue V=>new(Values[0],Values[1],Values[2],Values[3],Values[4],Dec(Values[5]),Dec(Values[6]),Dec(Values[7]),Dec(Values[8]),Dec(Values[9]),Dec(Values[10]),Dec(Values[11]),Dec(Values[12]),Values[13],Values[14],Values[15]); public NodeEditForm(DataRow? r,decimal feeRate,string? presetType=null):base(r==null?($"新增{presetType ?? "节点"}"):"修改节点", new FieldSpec("主仓编号","combo",Enumerable.Range(1,10).Select(i=>$"M{i}").ToArray(),S(r,"main_code","M1")), new FieldSpec("节点编号","combo",CodesFor(presetType),S(r,"code",DefaultCode(presetType))), new FieldSpec("类型","combo",new[]{"加仓","减仓","对冲","移动止盈","成本止损","止盈","止损"},S(r,"node_type",presetType ?? "加仓")), new FieldSpec("方向","combo",new[]{"多","空"},S(r,"direction","多")), new FieldSpec("状态","combo",new[]{"计划中","已开仓","已平仓","已取消","待归类"},S(r,"status","计划中")), new FieldSpec("数量","text",Default:S(r,"qty","")), new FieldSpec("开仓价","text",Default:S(r,"entry_price","")), new FieldSpec("平仓价","text",Default:S(r,"close_price","")), new FieldSpec("手续费","text",Default:S(r,"fee","")), new FieldSpec("资金费","text",Default:S(r,"funding","")), new FieldSpec("盈亏平衡","text",Default:S(r,"breakeven_price","")), new FieldSpec("成本止损","text",Default:S(r,"cost_stop","")), new FieldSpec("移动止盈","text",Default:S(r,"trailing_take_profit","")), new FieldSpec("开仓订单号","text",Default:S(r,"exchange_open_order_id","")), new FieldSpec("平仓订单号","text",Default:S(r,"exchange_close_order_id","")), new FieldSpec("备注","text",Default:S(r,"note",""))){} static string[] CodesFor(string? t)=>t switch{ "加仓"=>new[]{"A1","A2","A3","A4","A5"}, "减仓"=>new[]{"R1","R2","R3","R4","R5"}, "对冲"=>new[]{"H1","H2","H3","H4","H5"}, "移动止盈"=>new[]{"TP1","TP2","TP3","TP4","TP5"}, "成本止损"=>new[]{"SL1","SL2","SL3","SL4","SL5"}, _=>new[]{"A1","A2","A3","A4","A5","H1","H2","H3","TP1","TP2","TP3","SL1","SL2","R1","R2"}}; static string DefaultCode(string? t)=>CodesFor(t).First(); static string S(DataRow? r,string c,string d)=>r==null?d:Convert.ToString(r[c])??d; static decimal Dec(string s)=>decimal.TryParse(s,NumberStyles.Any,CultureInfo.InvariantCulture,out var d)?d:0m; }
